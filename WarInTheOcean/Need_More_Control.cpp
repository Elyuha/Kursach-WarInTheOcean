...
bool Need_More_Control::set_ship(char i, char j) {
...
bool Need_More_Control::choise_ships(char a) {
	int aa = a - 49;
	if (aa < 0 || aa > 3)
		return false;
	if (ships[aa] == 0)
...
bool Need_More_Control::attack_enemy(char i, char j) {
	if (i < 48 || i > 58 || j < 97 || j > 107)
		return false;
	int ii = i - 48;
	int jj = j - 97;
